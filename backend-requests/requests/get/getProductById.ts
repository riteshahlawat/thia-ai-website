 ...
 */
export class GetProductByIDBackendRequest implements IBackendRequest {
    backendRequest: AxiosInstance;
    actionName: string;

    constructor(backendRequest: AxiosInstance) {
        this.backendRequest = backendRequest;
        this.actionName = 'Get Product By ID';
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    run = async (config?: AxiosRequestConfig<any>, data?: [string, GetProductByIDData]): Promise<[boolean, any]> => {
        ...
        try {
            ...
            const res = await this.backendRequest.get(`/users/product/${data[1]}`, extendedAxiosConfig);
