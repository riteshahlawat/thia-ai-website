...
const Subscription = ({subscription}: any) => {
    ...
    const handleClick = async () => {
        if (user){
            ...
            const [isError, response] = await BackendRequestHandler.getInstance().cancelSubscriptionPlan(
                idToken,
                {
    ...
    return (
        <Box>
            ...
            <Box>
                Card Last 4 Numbers: {subscription.default_payment_method?.card?.last4}
            </Box>
...
const CreditCards = ({creditCards}: any) => {
    ...
    return (
        <Box>
            ...
            <Box>
                Card Last 4 Numbers: {creditCards.card.last4}
            </Box>
...
const Billing = (props: Props) => {
    ...
    useEffect(() => {
        const fetchSubscriptionAndCreditCards = async () => {
            if (!user) {
                ...
                const idToken = await user.getIdToken();
                const [isError, response] = await BackendRequestHandler.getInstance().listSubscriptionPlan(
                    idToken,
                    {
                        uid: user.uid,
                    }
                ...
                const [isError2, response2] = await BackendRequestHandler.getInstance().listCreditCards(
                    idToken,
                    {
                        uid: user.uid
                    }
    ...
    const useHandleStandardSubscription = async () => {
        ...
        if (user){
            const idToken = await user.getIdToken();
            const [isError, response] = await BackendRequestHandler.getInstance().subscribeStandardPlan(
                idToken,
                {
                    uid: user.uid
                }
    ...
    const useHandlePremiumSubscription = async () => {
        ...
        if (user){
            const idToken = await user.getIdToken();
            const [isError, response] = await BackendRequestHandler.getInstance().subscribePremiumPlan(
                idToken,
                {
                    uid: user.uid
                }
                ...
                <Popover>
                    ...
                    <Portal>
                        <PopoverContent color='white' bg='thia.purple.400' borderColor='thia.purple.400'>
                            <PopoverArrow />
                            <PopoverHeader><Center>Confirm your subscription plan!</Center></PopoverHeader>
                            <PopoverCloseButton />
                            <PopoverBody>
                                <Center>
